...
# 修改坐标轴刻度
plt.xticks(range(len(a)), a, fontproperties=font_ticks, rotation=45)
# 修改坐标轴和抬头
plt.xlabel("电影名称", fontproperties=font_label)
plt.ylabel("票房（亿）", fontproperties=font_label)
plt.title("2017年电影票房情况", fontproperties=font_title)
# x轴数据不能直接传入字符串，所以用数字代替
# width可以调节条形的宽度
plt.bar(range(len(a)), b, width=0.3, color='r')
# 保存图像
plt.savefig("./电影票房（竖排）.png")
# 绘图
plt.show()
